...
#elseif canImport(AppKit)
public protocol ImpressionProtocol: NSView {}
#endif

#if canImport(UIKit)
extension UIView: ImpressionProtocol {}
#elseif canImport(AppKit)
extension NSView: ImpressionProtocol {}
#endif
...
public extension ImpressionProtocol {
    ...
    #elseif canImport(AppKit)
    typealias ImpressionCallback<ViewType: NSView> = (_ view: ViewType, _ state: ImpressionState) -> ()
    #endif
...
#elseif canImport(AppKit)
    internal func getCallback() -> ImpressionCallback<NSView>? {
        return objc_getAssociatedObject(self, &impressionCallbackKey) as? ImpressionCallback<NSView>
    }
#endif
}
    ...
    private func areaRatio() -> Float {
        ...
        if let window = self as? UIWindow,
           self.superview == nil {
            // It's a root window
            #if os(visionOS)
            let intersection = self.frame.intersection( window.bounds)
            #else
            ...
            #if os(visionOS)
            let intersection = frameInScreen.intersection(window.bounds)
            #else
...
extension NSView {
    ...
    fileprivate func hookDidMoveToWindowIfNeeded() {
        ...
        self.hookingDidMoveToWindowToken = try? hookAfter(object: self, selector: #selector(NSView.viewDidMoveToWindow), closure: { view, _ in
            if view.window != nil {
                if !view.impressionState.isImpressed {
                    view.impressionState = .unknown
                }
        ...
        } as @convention(block) (NSView, Selector) -> Void)
    }
    ...
    fileprivate func rehookViewDidDisappearIfNeeded() {
        ...
        var hookingViewDidDisappearToken: Token?
        hookingViewDidDisappearToken = try? hookAfter(object: vc, selector: #selector(NSViewController.viewDidDisappear)) { [weak self] in
            guard let self = self,
                  self.isRedetectionOn(.viewControllerDidDisappear) else {
                hookingViewDidDisappearToken?.cancelHook()
                return
    ...
    fileprivate func readdNotificationObserver() {
        ...
        var names = [Notification.Name]()
        if self.isRedetectionOn(.didEnterBackground) {
            names.append(NSApplication.didHideNotification)
        }
        if self.isRedetectionOn(.willResignActive) {
            names.append(NSApplication.willResignActiveNotification)
        }
        guard names.count > 0 else {
            return
        }
        ...
        for name in names {
            let token = NotificationCenter.default.addObserver(forName: name, object: nil, queue: nil, using: {[weak self] notification in
                ...
                if notification.name == NSApplication.didHideNotification {
                    self.impressionState = .didEnterBackground
    ...
    private func areaRatio() -> Float {
        let alphaThreshold = CGFloat(self.alphaThreshold ?? NSView.alphaThreshold)
        guard self.isHidden == false && self.alphaValue >= alphaThreshold else {
            return 0
        }
        if let window,
           self.superview == nil {
            // It's a root window
            let intersection = self.frame.intersection( window.screen?.frame ?? .zero)
            let ratio = (intersection.width * intersection.height) / (self.frame.width * self.frame.height)
            return self.fixRatioPrecision(number: Float(ratio))
        } else {
            // It's normal view
            guard let window = self.window,
                  window.isVisible == false && window.alphaValue >= alphaThreshold else {
                return 0
            }
            // If super view hidden or alpha < alphaThreshold, self can't show
            var aView = self
            var frameInSuperView = self.bounds
            while let superView = aView.superview {
                guard superView.isHidden == false && superView.alphaValue >= alphaThreshold else {
                    return 0
                }
                frameInSuperView = aView.convert(frameInSuperView, to: superView)
                if aView.clipsToBounds {
            ...
            let frameInScreen = CGRect.init(x: frameInWindow.origin.x + window.frame.origin.x,
                                            ...
                                            width: frameInWindow.width,
                                            height: frameInWindow.height)
            let intersection = frameInScreen.intersection(window.screen?.visibleFrame ?? .zero)
            let ratio = (intersection.width * intersection.height) / (self.frame.width * self.frame.height)
            return self.fixRatioPrecision(number: Float(ratio))
        }
    }
    ...
    private func fixRatioPrecision(number: Float) -> Float {
        // As long as the different ratios on screen is within 0.01% (0.0001), then we can consider two ratios as equal. It's sufficient for this case.
        guard number > NSView.ratioPrecisionOffset else {
            return 0
        }
        guard number < 1 - NSView.ratioPrecisionOffset else {
            return 1
        }
        return number
    }
    ...
    private func detect() {
        ...
        // background
        if NSApplication.shared.isRunning == false {
            guard !self.isRedetectionOn(.didEnterBackground) else {
                self.impressionState = .didEnterBackground
                return
            }
        ...
        // If current view controller presented (non-full screen) a UIViewController, the viewDidDisappear of it will not be called. We need this logic to udpate the state.
        if let vc = self.parentViewController,
           vc.presentedViewControllers?.first != nil {
            guard !self.isRedetectionOn(.viewControllerDidDisappear) else {
                self.impressionState = .viewControllerDidDisappear
                return
            }
        ...
        // calculate
        let areaRatio = self.areaRatio()
        let areaRatioThreshold = self.areaRatioThreshold ?? NSView.areaRatioThreshold

        if case .inScreen(let fromDate) = self.impressionState {
            if areaRatio >= areaRatioThreshold {
                // keep appearance
                let interval = Date().timeIntervalSince(fromDate)
                let durationThreshold = self.durationThreshold ?? NSView.durationThreshold
                if Float(interval) >= durationThreshold {
                    // trigger impression
                    self.impressionState = .impressed(atDate: Date(), areaRatio: areaRatio)
    ...
    private func startTimer() {
        let timeInterval = TimeInterval(self.detectionInterval ?? NSView.detectionInterval)
        let timer = Timer.init(timeInterval: timeInterval, repeats: true, block: { [weak self] (timer) in
            // check self
            guard let self = self else {
                timer.invalidate()
            ...
            // check timer
            let currentTimeInterval = TimeInterval(self.detectionInterval ?? NSView.detectionInterval)
            guard currentTimeInterval.isEqual(to: timeInterval) else {
                self.stopTimer()
                self.startTimerIfNeeded()
                return
